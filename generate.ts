...
function applyTheme(
	doc: SVGElement,
	theme: Record<string, keyof typeof resources.colors>,
): void {
...
const filesWithDOM = resources.files.map(async (file) => {
	...
	return {
		...file,
		dom: new DOMParser().parseFromString(textFile, "image/svg+xml"),
	};
});
...
filesWithDOM.forEach(async (filePromise) => {
	...
	for (const theme of file.themes) {
		...
		applyTheme(
			file.dom as unknown as SVGElement,
			resources.themes[theme as ThemeKey] as Theme,
		);
		const domString = file.dom.toString();
		await Deno.writeTextFile(`./svg/${fileName}-${theme}.svg`, domString);
		await Promise.all(
			file.scale.flatMap(async (scale) => {
				return [
					Deno.writeFile(
						`./png/${fileName}-${theme}-x${scale}.png`,
						await svg2png(domString, {
							scale,
						}),
					),
				];
